...
def fetchBatFiles(dataTable,outPath,fileName = 'ep_flu.txt'):  # fetch individual files in Swift repository
    root = 'http://butler.lab.asu.edu/swift/$grb_id$/bat/' + fileName
    counter = 0
    missing = 0
    for row in dataTable:
...
def main():
    ...
    if len(sys.argv)!=3:
        print("""
        USAGE:
              ./readTable.py <web address: http://butler.lab.asu.edu/swift/bat_time_table.html> <path to output files, e,g,: ./>
              """)
    else:
        link = sys.argv[1]
        outPath = sys.argv[2]
